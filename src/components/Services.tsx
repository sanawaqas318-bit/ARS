    ...
    <section id="services" className="py-24 bg-background">
      <div className="container mx-auto px-6">
        <div className="text-center max-w-2xl mx-auto mb-20">
          ...
          <h2 className="font-headline text-4xl md:text-5xl font-black mb-6">Expertise Service! Let's check it out</h2>
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {services.map((service, index) => (
            <Card key={index} className={`bg-card border-none transition-all duration-300 hover:translate-y-[-10px] shadow-xl group ${index === 1 ? 'bg-primary text-black' : ''}`}>
              <CardContent className="p-10 text-center flex flex-col items-center">
                <div className={`mb-8 p-6 rounded-[2rem] transition-colors ${index === 1 ? 'bg-black text-primary' : 'bg-primary/10 text-primary group-hover:bg-primary group-hover:text-black'}`}>
                  {service.icon}
                </div>
                <h3 className="font-headline text-2xl font-black mb-4">{service.title}</h3>
                <p className={`leading-relaxed ${index === 1 ? 'text-black/70' : 'text-muted-foreground'}`}>
                  {service.description}
                </p>
              </CardContent>
            </Card>
